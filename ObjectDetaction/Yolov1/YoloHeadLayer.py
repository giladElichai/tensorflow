...
class YoloHeadLayer(Layer):
  ...
  def call(self, input):
    ...
    #class_probs = tf.nn.softmax(input[...,:C])
    # confs1 = tf.sigmoid( tf.reshape(input[..., C:C+1],[-1, S, S, 1]) )
    # box1_xy = tf.sigmoid( tf.reshape(input[..., C+1:C+3],[-1, S, S, 2]) )
    # box1_wh = tf.sigmoid( tf.reshape(input[..., C+3:C+5],[-1, S, S, 2]) ) * S

    # confs2 = tf.sigmoid( tf.reshape(input[..., C+5:C+6],[-1, S, S, 1]) )
    # box2_xy = tf.sigmoid( tf.reshape(input[..., C+6:C+8],[-1, S, S, 2]) )
    # box2_wh = tf.sigmoid( tf.reshape(input[..., C+8:],[-1, S, S, 2]) ) * S
